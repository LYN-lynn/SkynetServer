    ...
    skynet.dispatch("lua", function (_,_,id)
        ...
        if code then
            if code ~= 200 then  -- 如果协议解析有问题，就回应一个错误码 code 。
                ...
                local targetAssetPath = "/home/skynet/game/Assets/AB/login.unity3d"
                local fd = io.open(targetAssetPath, "r")
                local readContent = "fd:read(a)"
                skynet.error("返回code", code)
                skynet.error("返回内容", #readContent);
                response(id, code, readContent) --返回状态码200，并且跟上内容
            end
        else
