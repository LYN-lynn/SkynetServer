...
-- lyn new
local lasterror = ""
local lynerror = c.error
skynet.tracelog = c.trace
skynet.ferror = function (...)
...
skynet.error = function (...)
	local msg = debug.traceback()
	local start = string.find( msg, "game" )
	local tail = string.find( msg, "\n", start )
	if start ~= nil and tail~=nil then
		local last = string.sub( msg, start, tail - 1)
		if lasterror ~= last then
			lynerror("Trace----", last)
			lasterror = last
		end
		lynerror("---------", ...)
